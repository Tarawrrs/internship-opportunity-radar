...
POSITIVE_TITLE = re.compile(
    r"intern|research (assistant|associate)|economist|economic",
    re.I,
)
ALLOWED_LOCATION = re.compile(r"taipei|taiwan|singapore|japan|tokyo|asia|remote|multiple", re.I)
...
def make_job(*, source_id: str, title: str, company: str, location_raw: str, description: str, source: str, checked: str) -> dict:
...
def greenhouse_jobs(board: str, company: str, url: str, checked: str) -> list[dict]:
    ...
    for raw in data.get("jobs", []):
        ...
        if not POSITIVE_TITLE.search(title) or not ALLOWED_LOCATION.search(location):
            continue
...
def lever_jobs(board: str, company: str, url: str, checked: str) -> list[dict]:
    ...
    for raw in data:
        ...
        if not re.search(r"intern", commitment, re.I):
            continue
        if not POSITIVE_TITLE.search(title) or not ALLOWED_LOCATION.search(location):
            continue
...
def main() -> int:
    ...
    for kind, board, company, url in SOURCES:
        try:
            ...
            jobs.extend(fetched)
            source_status.append({"source": company, "status": "ok", "matches": len(fetched)})
        except (urllib.error.URLError, TimeoutError, ValueError, KeyError) as error:
            failures += 1
            source_status.append({"source": company, "status": "error", "message": type(error).__name__})
